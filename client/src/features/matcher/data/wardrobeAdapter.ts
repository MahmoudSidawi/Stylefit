...
export function wardrobeToGarment(item: WardrobeItem): Garment {
  ...
  return { id: item.id, name: item.name, brand: item.sample ? 'Sample Wardrobe' : 'Your Wardrobe',
    material: item.material, source: 'wardrobe', slot: category === 'bottoms' ? 'anchor' : category === 'dresses' ? 'dress' : 'core',
    ...
    drape: 'soft', occasions: ['work', 'weekend'], sample: item.sample }
}
