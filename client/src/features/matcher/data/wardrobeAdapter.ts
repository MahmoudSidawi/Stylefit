...
export function wardrobeToGarment(item: WardrobeItem): Garment {
  ...
  return { id: item.id, name: item.name, brand: 'Your Wardrobe',
    material: item.material, source: 'wardrobe', department: item.department, clothingType: item.kind, slot: category === 'shoes' ? 'shoes' : category === 'hats' ? 'hat' : category === 'bottoms' ? 'anchor' : category === 'dresses' ? 'dress' : 'core',
