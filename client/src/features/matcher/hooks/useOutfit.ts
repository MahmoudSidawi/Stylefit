  ...
  const garments: Garment[] = [...(catalogue.data?.items ?? []).flatMap((product) => {
    ...
    return [...new Set(variants.map((v) => v.color))].map((color) => {
      ...
      return { id: product.product_id + ':' + color, productId: product.product_id, name: product.name, brand: 'StyleFit', material: product.description,
        source: 'store' as const, department: product.department, clothingType: product.clothing_type, slot: product.category_id === 'shoes' ? 'shoes' as const : product.category_id === 'hats' ? 'hat' as const : product.category_id === 'bottoms' ? 'anchor' as const : product.category_id === 'dresses' ? 'dress' as const : 'core' as const,
