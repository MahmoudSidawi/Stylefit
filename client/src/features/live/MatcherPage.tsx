...
export default function LiveMatcher() {
  ...
  function toggle(key: string, value: MatchSelection) {
    ...
    setSelection((current) => {
      ...
      else if (Object.keys(next).length < 5) next[key] = value
      return next
    })
  }
  ...
  return <LiveLayout title="AI Outfit Matcher" privatePage>
    <p>Select two to five garments from the store, your wardrobe, or both. Groq will explain color, style, pattern, and clothing-type compatibility. Selected wardrobe photos are sent only when you run a check.</p>
    ...
    <fieldset disabled={busy}>
      ...
      <div className="live-actions"><strong>{count} of 5 pieces selected</strong><button className="button button-primary" disabled={count < 2 || busy} onClick={check}>{busy ? 'Checking with Groq…' : 'Check outfit with AI'}</button><button className="button button-surface" onClick={() => { setSelection({}); setResult(null) }}>Clear selection</button></div>
      <h2>Store clothes</h2><div className="live-grid">{products.data?.items.map((product) => {
        ...
        return <article className={`live-card${selected ? ' selected' : ''}`} key={product.product_id}>
          ...
          <button className="button button-surface" disabled={!variant || (!selected && count >= 5)} aria-pressed={selected} onClick={() => toggle(product.product_id, { source: 'store', variant_id: variantId })}>{selected ? 'Remove from outfit' : 'Select garment'}</button>
        </article>
      })}</div>
      <h2>Your wardrobe</h2><div className="live-grid">{wardrobe.data?.map((item) => {
        const selected = !!selection[item.wardrobe_item_id]
        return <article className={`live-card${selected ? ' selected' : ''}`} key={item.wardrobe_item_id}><h3>{item.name}</h3><p>{item.category_id} · {item.color}</p><button className="button button-surface" aria-pressed={selected} disabled={!selected && count >= 5} onClick={() => toggle(item.wardrobe_item_id, { source: 'wardrobe', wardrobe_item_id: item.wardrobe_item_id })}>{selected ? 'Remove from outfit' : 'Select garment'}</button></article>
      })}</div>
